...
export default function ServiceForm({ initialService, isMini = false }) {
  ...
  return (
    <div className={`${isMini ? 'h-full rounded-[1.75rem] p-4 md:rounded-[2rem] md:p-5' : 'rounded-[1.75rem] p-5 sm:rounded-[2rem] sm:p-6 md:rounded-[2.5rem] md:p-10'} group relative overflow-visible border border-slate-200 bg-slate-50 shadow-sm dark:border-white/10 dark:bg-white/5`}>
      <h3 className={`${isMini ? 'mb-4 text-[1.85rem] md:text-[2rem]' : 'mb-5 text-lg sm:text-xl md:mb-6'} font-heading font-bold text-slate-900 dark:text-white`}>
        Start your project
      </h3>

      <form onSubmit={handleSubmit} className={`${isMini ? 'space-y-3' : 'space-y-4'} relative z-10 w-full`}>
        ...
        <textarea required name="message" placeholder="Project details..." rows={isMini ? 2 : 3} value={formData.message} onChange={handleChange} className={`w-full resize-none rounded-2xl border border-slate-200 bg-white text-slate-900 ${isMini ? 'min-h-[84px] px-4 py-3 text-[15px]' : 'min-h-[110px] px-4 py-3.5 text-sm sm:min-h-0 sm:px-5 sm:py-4'} font-light shadow-sm outline-none transition-all focus:border-primary/50 dark:border-white/10 dark:bg-dark-bg/50 dark:text-white`} />

        <div className={`w-full rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-dark-bg/60 ${isMini ? 'mb-1 px-2.5 py-2.5' : 'mb-3 px-2.5 py-3 sm:mb-4 sm:px-4 sm:py-4'}`}>
