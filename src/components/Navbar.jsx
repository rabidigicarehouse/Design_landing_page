...
const Navbar = () => {
  ...
  const currentLogo = isDarkMode || headerOnHero ? '/assets/Digiicare_D.png' : '/assets/Digiicare.png';
    ...
    >
      <div className="container laptop-scale-navbar mx-auto flex items-center justify-between px-5 sm:px-6 lg:pl-10 lg:pr-6 xl:pl-12 xl:pr-7 2xl:px-6">
        <a 
          ...
          className="group relative flex h-14 items-center gap-3 sm:h-16 lg:ml-2 lg:h-[2.8rem] lg:max-w-[200px] xl:ml-3 xl:h-[3.2rem] xl:max-w-[240px] 2xl:h-[3.8rem] 2xl:max-w-[280px]"
        >
          <img src={currentLogo} alt="DigiCareHouse" className="h-full w-full max-w-[220px] object-contain object-left transition-all duration-700 group-hover:scale-105" />
        </a>
        ...
        <div className="flex items-center gap-4 lg:gap-3.5 xl:gap-4.5">
          ...
          <a href="#contact" onClick={(e) => handleScrollTo(e, '#contact')} className="hidden sm:block">
            <Button variant="primary" className="group rounded-full px-4 py-2 text-[6px] font-black uppercase tracking-[0.12em] shadow-lg shadow-primary/20 hover:scale-110 lg:py-2 xl:px-5 xl:py-2.25 xl:text-[7px] xl:tracking-[0.15em] 2xl:px-8 2xl:py-3 2xl:text-[10px] 2xl:tracking-widest">
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 left-0 top-0 z-[100] h-screen w-full bg-white p-4 dark:bg-dark-bg lg:hidden"
          >
            <div className="mx-auto flex h-full w-full max-w-sm flex-col rounded-[2rem] border border-black/5 bg-white px-6 pb-8 pt-6 shadow-[0_30px_80px_rgba(15,23,42,0.10)] dark:border-white/10 dark:bg-[#07101d]">
              <div className="mb-6 flex items-center justify-between gap-2">
                <div className="flex items-center gap-3">
                  <img src={currentLogo} alt="DigiCareHouse" className="h-12 w-auto object-contain" />
                </div>
                ...
                {navLinks.map((link, i) => {
                  ...
                  return (
                    <motion.a
                      ...
                      className="group flex items-center justify-between rounded-[1.4rem] border border-black/5 bg-slate-50/90 px-5 py-4 font-heading text-[2.15rem] font-black uppercase tracking-tighter text-slate-950 transition-all hover:border-primary/20 hover:bg-primary/5 hover:text-primary dark:border-white/8 dark:bg-white/[0.03] dark:text-white dark:hover:text-primary"
                      onClick={(e) => {
                        if (isSection && isHomePage) {
                          handleScrollTo(e, link.href);
                        }
